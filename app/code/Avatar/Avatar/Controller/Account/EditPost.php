...
namespace Avatar\Avatar\Controller\Account;
    ...
    public function execute()
    {
        ...
        if ($validFormKey && $this->getRequest()->isPost()) {
            ...
            try {
                ...
                // Upload customer avatar and save in custom attribute
                try {
                    if ($this->session->isLoggedIn()) {
                        $file = get_object_vars($this->getRequest()->getFiles());
                        if(!empty($file['profile_picture']['name']) && !empty($file['profile_picture']['type'])) {
                            if($this->helper->validateFileFormat($file['profile_picture']['type'])) {
                                if ($this->helper->validateFileSize($file['profile_picture']['size'])) {
                                    $uploaddir = $_SERVER['DOCUMENT_ROOT'] . '/media/customer/';
                                    $uploadfile = $uploaddir . $file['profile_picture']['name'];
                                    move_uploaded_file($_FILES['profile_picture']['tmp_name'], $uploadfile);
                                    $customerCandidateDataObject->setCustomAttribute('profile_picture', $file['profile_picture']);
                                    $this->messageManager->addSuccessMessage('Avatar Has Been Updated Successfully');
                                }
                                else{
                                    throw new FilesystemException('Too big a file!');
                                }
                            }
                            else{
                                throw new FilesystemException('Wrong file format !');
                            }
                        }
                    }
